...
export function refetchProject(dispatch: any, router: any) {
  if (checkSession()) {
    const date = new Date()
    ProjectList()
      .then((res) => {
...
export function project_initial(current_project: any) {
  if (current_project) {
    return {
...
export function project_dateRange(current_project: any) {
  if (current_project) {
    return [
      {
        text: '기초 기획안 작성',
